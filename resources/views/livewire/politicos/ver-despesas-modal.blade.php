        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" wire:click.self="$set('isOpen', false)">
            ...
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[85vh] flex flex-col" @click.outside="$set('isOpen', false)" @keydown.escape.window="$set('isOpen', false)">
                <div class="flex items-center justify-between px-6 py-4 border-b border-zinc-200">
                    <flux:heading size="lg">Todas as Despesas</flux:heading>
                ...
                <div class="px-6 py-3 border-b border-zinc-100">
                    <input type="text" wire:model.live.debounce.300ms="search" placeholder="Buscar despesa..." class="w-full px-3 py-2 text-sm border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>

                <div class="flex-1 overflow-y-auto px-6 py-3">
                    @if ($expenses->isEmpty())
                        <p class="text-sm text-zinc-500 text-center py-8">Nenhuma despesa encontrada.</p>
                    @else
                        <div class="space-y-2">
                            @foreach ($expenses as $expense)
                                <div class="flex items-center gap-3 bg-white border border-zinc-200 rounded-lg px-4 py-3">
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-xs text-zinc-500">{{ $expense->document_date?->format('d/m/Y') }} · {{ $expense->type }}</p>
                                    </div>
